...
def FMD_AB(pythonbin, inputfile):
    ...
    output = subprocess.check_output(f'arch -arm64 "{pythonbin}" "{pyfile}" "{inputfile}" "{ABfile}" "{outputfile}"', shell=True)
    return output.decode(), outputfile
...
def ABvalue(pythonbin, inputfile):
    ...
    output = subprocess.check_output(f'arch -arm64 "{pythonbin}" "{pyfile}" "{inputfile}" "{MCfile}" "{outputfile}"', shell=True)
    return output.decode(), outputfile

def plot_MTdensity(inputfile):
...
def area_cut(pythonbin, inputfile, outputdir, area_geoms, dict_depth):
    tmp_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "tmp")
    os.makedirs(tmp_dir, exist_ok=True)
    ...
    output = subprocess.check_output(f'arch -arm64 "{pythonbin}" "{pyfile}" "{inputfile}" "{outputfile}" "{outputdir}"', shell=True)
    return output.decode(), outputfile